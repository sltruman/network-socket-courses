...
bool Authorization::verity() {
    QSettings settings(GLOBAL_COMPANY,GLOBAL_NAME);
    QString authorizationFilePath = settings.value("authorizationFilePath").toString();
    QFile authorizationFile(authorizationFilePath.trimmed());
    authorizationFile.open(QFile::ReadOnly);
    QByteArray keydata = authorizationFile.readAll();
    authorizationFile.close();

    for(int i=0;i<keydata.length();i++)
        keydata[i] = keydata[i] ^ 'c';

    Json::Reader reader;
    Json::Value authorizationKey;

    if(!reader.parse(keydata.data(),authorizationKey)) {
        std::cout << QString::fromLocal8Bit("[Authorization] 解析授权文件失败！").data() << std::endl;
        return false;
    }

    QByteArray authorizationCode = QByteArray::fromHex(authorizationKey["authorization"].asCString());
    uint deadline = static_cast<uint>(authorizationKey["deadline"].asUInt());
    QString flag = QString::fromLocal8Bit(authorizationKey["flag"].asCString());
    QByteArray publicKeyData = QByteArray::fromHex(authorizationKey["public"].asCString());
    settings.setValue("flag",flag);
    uint8_t* publicKey = reinterpret_cast<uint8_t*>(publicKeyData.data());
    QByteArray md5 = QCryptographicHash::hash(getHostmacAddress().toLocal8Bit(), QCryptographicHash::Md5);
    uint lastTime = settings.value("lastTime",0).toUInt();
    uint time = QDateTime::currentDateTime().toTime_t();

    if(time < lastTime) {
        std::cout << "[Authorization] 系统时间错误!" << std::endl;
        return false;
    }

    settings.setValue("lastTime",time);
    std::cout << "[Authorization] 授权机器：" << md5.toHex().data() << std::endl;
    std::cout << "[Authorization] 授权标识：" << flag.toLocal8Bit().data() << std::endl;
    std::cout << "[Authorization] 截止时间：" << QDateTime::fromTime_t(deadline).toString("yyyy/M/d HH:mm").toLocal8Bit().data() << std::endl;

    QString address = "http://www.dungbeetles.xyz:8000/three-qt/heart?id=%1&flag=%2&deadline=%3";
    address = address.arg(md5.toHex().data()).arg(flag).arg(deadline);
    http.get(QNetworkRequest(QUrl(address)));

    return time < deadline && ecdsa_verify(publicKey,reinterpret_cast<uint8_t*>(md5.toHex().data()),reinterpret_cast<uint8_t*>(authorizationCode.data()));
}
